...
// Tableau vide pour revenir au style par défaut (clair)
...
function toggleTheme() {
    ...
    if (isDark) {
        ...
        if (btn) btn.textContent = '🌙 Passer en mode nuit';
        if (map && typeof map.setOptions === 'function') {
            map.setOptions({ styles: [] }); // Restaure le style par défaut
        }
    } else {
        // Passage au mode sombre
        body.setAttribute('data-theme', 'dark');
        if (btn) btn.textContent = '☀️ Passer en mode clair';
        if (map && typeof map.setOptions === 'function') {
            map.setOptions({ styles: darkMapStyles }); // Applique les styles sombres natifs
        }
    }
}

// Initialisation de l'écouteur au chargement (à mettre en fin de script ou dans le DOMContentLoaded)
